...
| 전략           | 설명                                                          | 특징                         |
| -------------- | ------------------------------------------------------------- | ---------------------------- |
| **Rolling**    | 구버전 인스턴스를 순차적으로 신버전으로 교체                  | 무중단, 리소스 절약           |
| **Blue/Green** | 구버전(Blue)과 신버전(Green) 환경을 동시에 유지, 트래픽 전환  | 즉시 롤백 가능, 리소스 2배   |
| **Canary**     | 일부 사용자에게만 신버전을 먼저 배포하여 검증                 | 리스크 분산, 점진적 배포     |

- [배포 전략: Rolling, Blue/Green, Canary](https://onlywis.tistory.com/10)

## 관련 노트

- [Docker](/notes/Docker/) - 컨테이너 기반 배포
- [Kubernetes](/notes/Kubernetes/) - 컨테이너 오케스트레이션 (Rolling/Canary 배포 지원)
